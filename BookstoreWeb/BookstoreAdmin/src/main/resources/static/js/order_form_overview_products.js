...
function getNumberValueRemovedThousandSeparator(fieldRef) {
    // Lấy giá trị từ fieldRef
    let fieldValue = fieldRef.val();

    // Xóa dấu chấm (.) nếu nó là dấu phân cách phần nghìn
    // Giả sử số có định dạng "1.234.567,89" (dấu chấm cho phần nghìn, dấu phẩy cho phần thập phân)
    // Thay dấu chấm (.) bằng chuỗi rỗng
    fieldValue = fieldValue.replace(/\./g, '');

    // Thay dấu phẩy (,) bằng dấu chấm (.) để chuẩn hóa cho parseFloat
    fieldValue = fieldValue.replace(/,/g, '.');
